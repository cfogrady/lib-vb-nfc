...
class VBNfcHandlerTest {
    ...
    fun validateDataDecryption() {
        ...
        // val pagesOfInterst = listOf<IntRange>(8..223) // everything
        val pagesOfInterst = listOf<IntRange>(24..103) // character data
        // val pagesOfInterst = listOf<IntRange>(40..47, 74..79) // same character data
        ...
        var result = vbNfcHandler.decryptData(characterData, testTagId)
        vbNfcHandler.validateCharacterData(result)
        var nfcCharacter = nfcCharacterFactory.buildNfcCharacterFromBytes(result, 4u)
        println(nfcCharacter)
        printShortData(result, pagesOfInterst)
        printByteData(result, pagesOfInterst)
        ...
        result = vbNfcHandler.decryptData(characterData, testTagId)
        vbNfcHandler.validateCharacterData(result)
        nfcCharacter = nfcCharacterFactory.buildNfcCharacterFromBytes(result, 4u)
        println(nfcCharacter)
        printShortData(result, pagesOfInterst)
        printByteData(result, pagesOfInterst)
        validateCopies(result)
    ...
    fun validateCopies(data: ByteArray) {
        for (range in copies) {
            for(i in (range.first-8)*4..<(range.last - 8) * 4) {
