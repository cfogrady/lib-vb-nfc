...
class VBNfcHandlerTest {
    ...
    fun printByteData(data: ByteArray) {
        for(i in data.indices step 4) {
            val page = String.format("%03d", i/4+8)
            val stringBuilder = StringBuilder("Page $page: ")
            for(j in 0..<4) {
    ...
    fun printShortData(data: ByteArray) {
        for(i in data.indices step 4) {
            val page = String.format("%03d", i/4+8)
            val stringBuilder = StringBuilder("Page $page: ")
            for(j in 0..2 step 2) {
